...
import com.example.zakataceh.databinding.ActivityMainBinding
import com.example.zakataceh.db.NoteHelper
import com.example.zakataceh.entity.Note
import com.example.zakataceh.helper.MappingHelper
...
class MainActivity : AppCompatActivity() {
        ...
        if (result.data != null) {
            ...
            when (result.resultCode) {
                NoteAddUpdateActivity.RESULT_ADD -> {
                    ...
                    showSnackbarMessage("Penerima zakat berhasil ditambahkan")
                }
                NoteAddUpdateActivity.RESULT_UPDATE -> {
                    ...
                    showSnackbarMessage("Penerima zakat berhasil diubah")
                }
                NoteAddUpdateActivity.RESULT_DELETE -> {
                    val position = result?.data?.getIntExtra(NoteAddUpdateActivity.EXTRA_POSITION, 0) as Int
                    adapter.removeItem(position)
                    showSnackbarMessage("Penerima zakat berhasil dihapus")
                }
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "Penerima Zakat"
